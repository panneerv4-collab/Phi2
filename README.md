...
1. **An interactive card editor** (`index.html`) — type in a problem, watch a
   realistic notebook-page card build itself: ruled lines, a red margin rule,
   binder holes, a dashed "GIVEN" box, a yellow highlighter marker over the
   key result, and a hand-wobbled red-pen circle around the final answer,
   complete with a teacher's ✓ stamp.
2. **An automation endpoint** (`templates/mathematics-card.html`) — the same
   card, driven entirely by URL query parameters, with no editor chrome. Point
   ScreenshotOne at it and you get a clean 1080×1350 PNG, ready to post.

No build step, no framework, no server-side code. Everything is plain
...
Type a problem into the left panel and watch the card update live. Use the
theme swatches to switch the pen/highlighter colors between **Navy/Red**,
**Emerald**, and **Plum**.
